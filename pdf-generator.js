...
try {
    ...
    // PO Number (left) and Date (right) with underlines
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    ...
    doc.text(nameLines, supplierValueX, yPosition);
    const nameHeight = nameLines.length * 5;
    doc.line(supplierValueX, yPosition + 0.5, rightMargin, yPosition + 0.5);
    ...
    doc.text(addressLines, supplierValueX, yPosition);
    ...
    doc.rect(tableX, yPosition, tableWidth, headerHeight);
    
    // Draw vertical lines and add headers
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    ...
    doc.text('S. NO.', currentX + 2, yPosition + 5.5);
    currentX += col1Width;
    
    // ITEM
    doc.line(currentX, yPosition, currentX, yPosition + headerHeight);
    doc.text('ITEM', currentX + 2, yPosition + 5.5);
    currentX += col2Width;
    
    // QUANTITY
    doc.line(currentX, yPosition, currentX, yPosition + headerHeight);
    doc.text('QUANTITY', currentX + 2, yPosition + 5.5);
    currentX += col3Width;
    
    // RATE
    doc.line(currentX, yPosition, currentX, yPosition + headerHeight);
    doc.text('RATE', currentX + 2, yPosition + 5.5);
    currentX += col4Width;
    
    // REMARKS
    doc.line(currentX, yPosition, currentX, yPosition + headerHeight);
    doc.text('REMARKS', currentX + 2, yPosition + 5.5);
    currentX += col5Width;
    ...
    // Table Row (Data) - calculate content first to determine row height
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);

    const itemLines = doc.splitTextToSize(po.itemDescription, col2Width - 4);
    const qtyLines = doc.splitTextToSize(po.quantity, col3Width - 4);
    const rateText = `Rs ${po.rate}`;
    const rateLines = doc.splitTextToSize(rateText, col4Width - 4);
    const remarkLines = doc.splitTextToSize(po.remarks || '', col5Width - 4);
    ...
    const rowHeight = Math.max(35, maxContentHeight + cellPadding * 2);
    ...
    doc.text('1', currentX + 5, yPosition + cellPadding + lineHeight);
    currentX += col1Width;

    // ITEM
    doc.line(currentX, yPosition, currentX, yPosition + rowHeight);
    doc.text(itemLines, currentX + 2, yPosition + cellPadding);
    currentX += col2Width;

    // QUANTITY
    doc.line(currentX, yPosition, currentX, yPosition + rowHeight);
    doc.setFont('helvetica', 'bold');
    doc.text(qtyLines, currentX + 2, yPosition + cellPadding);
    currentX += col3Width;

    // RATE
    doc.line(currentX, yPosition, currentX, yPosition + rowHeight);
    doc.text(rateLines, currentX + 2, yPosition + cellPadding);
    currentX += col4Width;

    // REMARKS
    doc.line(currentX, yPosition, currentX, yPosition + rowHeight);
    doc.setFont('helvetica', 'normal');
    doc.text(remarkLines, currentX + 2, yPosition + cellPadding);
    currentX += col5Width;
    ...
    doc.setFont('helvetica', 'bold');
    const deliveryX = margin + 30;
    doc.text(po.deliveryDate, deliveryX, yPosition);
    doc.line(deliveryX, yPosition + 0.5, deliveryX + doc.getTextWidth(po.deliveryDate), yPosition + 0.5);
    ...
    // Save PDF
    const fileName = `PO_${po.poNumber}_${po.supplierName.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
    doc.save(fileName);
}